"""Jaimini Chara Dasha father death prediction pipeline.

Sign-based dasha system where each sign rules for 1-12 years.
Uses Chara Dasha Pratyantar (depth=3, ~5-30 day periods) as prediction unit.
Pure Jaimini features: Rashi Drishti, Chara Karakas, Karakamsha, Argala,
Arudha Pada + universal transit features.

NO Parashari dasha features, NO KP, NO BaZi — Jaimini tradition only.

Usage:
    python -u -m ml.pipelines.father_death_predictor.run_jaimini
"""
...
DATA_DIR = PROJECT_ROOT / 'data' / 'jaimini'
...
def _make_adapted(cand, jai_ctx):
    """Create adapted candidate with planet lords for shared modules.

    Chara Dasha: lords = [sign_names], signs = [sign_indices]
    Shared modules expect: lords = [planet_names]
    """
    adapted = {**cand}
    adapted['lords'] = [get_sign_lord(s) for s in cand['signs']]
    # Set maraka_type for modules that check it
    pd_lord = adapted['lords'][-1]
    adapted['maraka_type'] = (
        'primary' if pd_lord in jai_ctx['father_maraka_lords'] else 'none')
    return adapted


def extract_features_for_candidate(cand, chart, asc, jai_ctx, gc_ctx,
                                    candidates, ci,
                                    depth1_periods, depth2_periods,
                                    jup_bav, hi_ctx):
    """Extract Jaimini + transit features for a Chara Dasha PD candidate."""
    from .features.jaimini_features import (
        extract_jaimini_features, extract_jaimini_subperiod_features,
        extract_jaimini_transit_features)
    ...
    adapted = _make_adapted(cand, jai_ctx)

    # --- Jaimini-native features (~60) ---
    try:
        jai_f = extract_jaimini_features(cand, jai_ctx, chart)
        f.update(jai_f)
    except Exception:
        pass

    # --- Jaimini sub-period density (~10) ---
    try:
        jsk_f = extract_jaimini_subperiod_features(cand, jai_ctx)
        f.update(jsk_f)
    except Exception:
        pass

    # --- Jaimini transit interactions (~25) ---
    try:
        jt_f = extract_jaimini_transit_features(cand, jai_ctx, chart)
        f.update(jt_f)
    except Exception:
        pass

    # --- Shared transit features at midpoint (~80) ---
    gc_f = {}
    try:
    ...
    # --- Nakshatra of PD sign lord ---
    try:
        father_mk_lords = jai_ctx['father_maraka_lords']
        nk_f = extract_nakshatra_features(adapted, chart, father_mk_lords)
        f.update(nk_f)
    except Exception:
        pass
    ...
    # --- Lord transit (Saturn/Jupiter on PD sign lord's natal degree) ---
    try:
    ...
    # --- Hierarchy features (MD/AD/PD sign lord interactions) ---
    hi_f = {}
    try:
    ...
    try:
        jai_hi = {
            'hi_maraka_cascade': hi_f.get('hi_maraka_cascade',
                                          f.get('jai_maraka_cascade_3', 0)),
            'hi_danger_cascade': hi_f.get('hi_danger_cascade',
                                          f.get('jai_danger_cascade_3', 0)),
            'hi_md_is_maraka': hi_f.get('hi_md_is_maraka',
                                        f.get('jai_md_is_maraka', 0)),
            'hi_md_ad_both_maraka': hi_f.get('hi_md_ad_both_maraka',
                                             f.get('jai_both_ad_pd_maraka', 0)),
            'hi_md_pd_combined': hi_f.get('hi_md_pd_combined', 0),
        }
        cx_f = extract_cross_features(gc_f, jai_hi)
        f.update(cx_f)
    except Exception:
        pass

    # --- Lord discrimination (PD sign lord natal strength) ---
    try:
    ...
    try:
        d9_f = extract_navamsha_features(
            adapted, chart, asc, jai_ctx['father_maraka_lords'])
        f.update(d9_f)
    except Exception:
        pass
...
def build_jaimini_dataset(records, name, start_index, n_augment=5):
    """Build Jaimini Chara Dasha PD depth=3 dataset."""
    from .astro_engine.ephemeris import compute_chart, compute_jd
    from .astro_engine.chara_dasha import compute_chara_dasha
    from .features.dasha_window import construct_dasha_window
    from .features.gochar_features import precompute_gochar_context
    from .features.jaimini_features import precompute_jaimini_context
    ...
    print(f"  Building {name} Jaimini Chara PD depth=3 "
    ...
    for i, rec in enumerate(records):
        ...
        try:
            ...
            # Chara Dasha at all depths
            chara_d3 = compute_chara_dasha(chart, asc, birth_jd, max_depth=3)
            chara_d2 = compute_chara_dasha(chart, asc, birth_jd, max_depth=2)
            chara_d1 = compute_chara_dasha(chart, asc, birth_jd, max_depth=1)
            target_periods = [p for p in chara_d3 if p['depth'] == 3]

            # Precompute contexts
            jai_ctx = precompute_jaimini_context(chart, asc)
            ...
            for aug in range(n_augment):
                ...
                for ci_pre, cand_pre in enumerate(candidates):
                    adapted_pre = _make_adapted(cand_pre, jai_ctx)
                    try:
                        tier, dscore = classify_candidate_extended(
                            adapted_pre, chart, asc, maraka_pre)
                    except Exception:
                        # Fallback: use Jaimini sign maraka status
                        pd_sign = cand_pre['signs'][-1]
                        is_mk = pd_sign in jai_ctx['father_maraka_signs']
                        is_dg = pd_sign in jai_ctx['father_danger_signs']
                        mk_count = sum(
                            s in jai_ctx['father_maraka_signs']
                            for s in cand_pre['signs'])
                ...
                for ci, cand in enumerate(candidates):
                    f = extract_features_for_candidate(
                        cand, chart, asc, jai_ctx, gc_ctx, candidates, ci,
                        chara_d1, chara_d2, jup_bav, hi_ctx)
                    row = {
                        'group_id': idx,
                        'cand_idx': ci,
    ...
    if len(df) > 0 and 'group_id' in df.columns:
        ...
        # Add Jaimini-specific + transit relative features
        JAIMINI_EXTRA_REL = [
            'jai_pd_lord_dignity', 'jai_pd_lord_uchcha',
            'jai_pd_dist_h9', 'jai_pd_argala_malefic',
            'jai_maraka_cascade_3', 'jai_danger_cascade_3',
            'jai_drishti_maraka_count',
            # Jaimini transit interactions (continuous)
            'jt_pd_lord_dist_sun', 'jt_pd_lord_dist_h9',
            'jt_sat_dist_pd_lord', 'jt_jup_dist_pd_lord',
            'jt_rahu_dist_pd_lord', 'jt_composite_danger',
            ...
            # Lord transit distance
            'lt_sat_pl_dist',
            # Jupiter BAV
            'jb_jup_bav',
            # D9
            'd9_dispositor_dignity',
        ]
        avail = [c for c in JAIMINI_EXTRA_REL if c in df.columns]
        for feat in avail:
...
def main():
    print("=" * 75)
    print("JAIMINI CHARA DASHA FATHER DEATH PREDICTION (PD depth=3, 24mo window)")
    print("=" * 75)
    ...
    df_tr = build_jaimini_dataset(train_recs, 'train', 0, n_augment=5)
    df_va = build_jaimini_dataset(val_recs, 'val', len(train_recs) * 100,
                                   n_augment=1)
    ...
    # In Chara Dasha: PD duration = MD_years / 144.
    # ALL PDs within same MD have equal duration.
    # So MD identity → duration. But PD/AD sign identity does NOT.
    # Exclude: duration, movement, ingress, sign_change, _is_min, _is_max,
    # and seq duration features.
    _DUR_PROXY = {
        'duration_days',
        # Sequence duration features (directly encode period length)
        'seq_seq_duration_days', 'seq_seq_dur_vs_mean',
        'seq_seq_dur_log', 'seq_seq_danger_intensity',
        # Sandhi days (absolute, correlates with AD/MD duration)
    ...
    # --- Duration correlation audit ---
    dur_vals = df_va_f['duration_days'].values
    print(f"\n  Duration correlation audit (|r|>0.10):")
    high_corr_feats = []
    for feat in no_dur_cols:
        try:
            r = np.corrcoef(df_va_f[feat].values, dur_vals)[0, 1]
            if abs(r) > 0.10:
                high_corr_feats.append((feat, r))
        except Exception:
            pass
    high_corr_feats.sort(key=lambda x: abs(x[1]), reverse=True)
    for feat, r in high_corr_feats[:20]:
        print(f"    {feat:45s} r={r:+.3f}")

    # Remove features with |r|>0.15 from no-dur set
    high_corr_names = {feat for feat, r in high_corr_feats if abs(r) > 0.15}
    if high_corr_names:
        print(f"\n  Removing {len(high_corr_names)} features with |r|>0.15:")
        for fn in sorted(high_corr_names):
            print(f"    - {fn}")
        no_dur_cols = [c for c in no_dur_cols if c not in high_corr_names]
        print(f"  No-dur features after cleanup: {len(no_dur_cols)}")

    # Train no-dur base model
    ...
    # ── Binary classifier blend with weight sweep ─────────────────────
    params_clf = {
    ...
    # ── Results ───────────────────────────────────────────────────────
    print(f"\n  Jaimini Chara PD (depth=3) Results:")
    ...
    print(f"    {'No-dur base':>25s} {nd_topk['top_1']:6.1%}  {nd_topk['top_3']:6.1%}  {nd_topk['top_5']:6.1%}  {nd_topk['top_1']/max(rnd_t1,0.001):.1f}x")
